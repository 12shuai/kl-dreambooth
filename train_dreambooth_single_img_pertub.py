...
class SpatialDreambooth:
    ...
    def main(self):
        ...
        assert not self.args.with_prior_preservation
        ...
        assert not self.args.train_token
        ...
        # We start by only optimizing the embeddings
        # learnable_vae_tensor=torch.zeros((1,4,64,64),dtype=self.weight_dtype).to(self.accelerator.device)
        learnable_vae_tensor=torch.zeros((1,4,64,64),dtype=torch.float32).to(self.accelerator.device)
        learnable_vae_tensor.requires_grad=True
        params_to_optimize = [learnable_vae_tensor]
        ...
        (
            ...
            learnable_vae_tensor,self.unet,self.text_encoder,optimizer, train_dataloader, lr_scheduler
        )
        ...
        for epoch in range(first_epoch, self.args.num_train_epochs):
            ...
            for step, batch in enumerate(train_dataloader):
                ...
                with self.accelerator.accumulate(self.unet):
                    ...
                    # print(learnable_vae_tensor.requires_grad,loss.grad,loss.requires_grad,"sxc")
                    if self.args.regular_term>0:
                        regular_term=self.args.regular_term*learnable_vae_tensor.pow(2).mean()
                        loss+=regular_term
                ...
                if self.accelerator.sync_gradients:
                    ...
                    if (
                        ...
                        torch.save(learnable_vae_tensor,os.path.join(ckpts_path,"vae.pth"))
                ...
                logs["vae_tensor_sum"]=learnable_vae_tensor.sum().item()
        ...
        self.accelerator.wait_for_everyone()
        torch.save(learnable_vae_tensor,self.args.output_dir)
