...
class AttentionStore(AttentionControl):
    ...
    def forward(self, attn, is_cross: bool, place_in_unet: str,**kwargs):
...
class AttentionValueStore(AttentionControl):
    @staticmethod
    def get_empty_store():
        return {
            "down_cross": [],
    ...
    def forward(self, attn, is_cross: bool, place_in_unet: str,**kwargs):
        ...
        value=kwargs["v"]
        # print(key,value.shape,attn.shape)
        if attn.shape[1] <= 32**2:
            self.step_store[key].append(attn)
            self.step_value[key].append(value)
        return attn

    def between_steps(self):
        if len(self.attention_store) == 0:
            self.attention_store = self.step_store
            self.global_value = self.step_value
        else:
            for key in self.attention_store:
                for i in range(len(self.attention_store[key])):
                    self.attention_store[key][i] += self.step_store[key][i]
                    self.global_value[key][i] += self.step_value[key][i]
        self.step_store = self.get_empty_store()
        self.step_value = self.get_empty_value()
    ...
    def get_average_value(self):
        average_attention = {
            key: [item / self.cur_step for item in self.global_value[key]]
            for key in self.global_value
        }
        return average_attention

    def reset(self):
        super(AttentionValueStore, self).reset()
        self.step_store = self.get_empty_store()
        self.attention_store = {}
        self.step_value = self.get_empty_value()
        self.global_value = {}
    ...
    def __init__(self):
        super(AttentionValueStore, self).__init__()
        self.step_store = self.get_empty_store()
        self.step_value=self.get_empty_value()
        self.attention_store = {}
        self.global_value={}
