...
class DreamBoothDataset(Dataset):
    ...
    def __init__(
        ...
        assert class_data_root is not None
...
class SpatialDreambooth:
    ...
    def main(self):
        logging_dir = Path(self.args.output_dir, self.args.logging_dir,self.args.proj_name)
        ...
        self.args.class_data_dir=os.path.join(self.args.class_data_dir,self.args.proj_name)
        ...
        num_update_steps_per_epoch = math.ceil(
            max(len(os.listdir(self.args.instance_data_dir)),self.args.num_class_images) / (self.args.gradient_accumulation_steps* self.args.train_batch_size)
        )
        ...
        if optimizer is not None:
            ...
            (
                ...
                self.unet, self.text_encoder, optimizer, lr_scheduler
            )
        else:
            (
                self.unet,
        ...
        logger.info(f"  Num batches each epoch = {num_update_steps_per_epoch}")
        ...
        assert self.args.with_prior_preservation, "wrong!!!!!!!!!"
        for epoch in range(first_epoch, self.args.num_train_epochs):
            if (epoch-first_epoch) % self.args.exchange_interval==0:
            # if (epoch-first_epoch)==0:
                train_dataloader=self._make_new_dataloader(epoch)
            ...
            for step, batch in enumerate(train_dataloader):
                ...
                with self.accelerator.accumulate(self.unet):
                    ...
                    print(latents.dtype,self.unet.dtype,noisy_latents.dtype)
                    # exit(0)
    ...
    def _make_new_dataloader(self,epoch):
        ...
        def generate_data():
            gen_images_dir = os.path.join(self.args.class_data_dir,str(epoch))
            if os.path.exists(gen_images_dir):
                shutil.rmtree(gen_images_dir)
            os.makedirs(os.path.join(self.args.class_data_dir,str(epoch)),exist_ok=True)
            torch_dtype = (
                torch.float16
            ...
            # print("torch_dtype:",torch_dtype,self.weight_dtype)
            ...
            pipeline.unet.load_state_dict(self.unet.state_dict())
            pipeline.text_encoder.load_state_dict(self.text_encoder.state_dict())
            ...
            with torch.no_grad():
                ...
                num_new_images = self.args.num_class_images
                ...
                pipeline.to(self.accelerator.device)
                print(pipeline.unet.dtype,pipeline.text_encoder.dtype,pipeline.vae.dtype)
                ...
                for example in tqdm(
                    ...
                    for i, (image,prompt) in enumerate(zip(images,example["prompt"])):
                        ...
                        image_filename = (
                            os.path.join(gen_images_dir,f"{hash_image}.jpg")
                        )
        ...
        train_dataset = DreamBoothDataset(
            ...
            instance_data_root=self.args.instance_data_dir,
            # placeholder_tokens=self.placeholder_tokens,
            class_data_root=os.path.join(self.args.class_data_dir,str(epoch)),
            class_prompt=self.args.class_prompt,
            tokenizer=self.tokenizer,
            size=self.args.resolution,
            center_crop=self.args.center_crop,
