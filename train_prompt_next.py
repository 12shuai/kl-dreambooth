...
def parse_args(input_args=None):
    ...
    parser.add_argument("--lambda_attention", type=float, default=0) ##1e-2
...
def get_customized_prompt_from_prompt(prompt):
    token_list=prompt.split(" ")
    return [token for token in token_list if token.startswith("<") and token.endswith(">")]
...
class DreamBoothDataset(Dataset):
    ...
    def __init__(
        ...
        self.flip_p = flip_p
        self.customized_prompt_list=customized_prompt_list
        ...
        assert len(instance_img_path_list)==len(customized_prompt_list)
        # self.instance_data_root = Path(instance_data_root)
        self.placeholder_tokens_list=[]
        self.placeholder_tokens_length_list=[]
    
            
        for customized_prompt in customized_prompt_list:
            self.placeholder_tokens_list.append(get_customized_prompt_from_prompt(customized_prompt))
            self.placeholder_tokens_length_list.append(len(self.placeholder_tokens_list[-1]))
        # self.placeholder_tokens = placeholder_tokens

        
        self.instance_image_list=[]
        self.instance_img_path_list=instance_img_path_list



        for instance_img_path in instance_img_path_list:
            image=Image.open(instance_img_path)
            if 'A' in image.getbands():
                image = image.convert('RGB')
            self.instance_image_list.append(self.image_transforms(image))

        self.instance_mask_list=[]
        for instance_img_path,placeholder_tokens_list in zip(self.instance_img_path_list,self.placeholder_tokens_list):
            self.instance_mask_list.append([])
            name,ext=os.path.splitext(instance_img_path)
            non_sense=torch.ones((1,1,512,512))
            for placeholder_tokens in placeholder_tokens_list:
                concept_name=placeholder_tokens.strip("<>").rstrip("0123456789")
                
                if os.path.exists(name+f"_{concept_name}_mask"+ext):
                    image=Image.open(name+f"_{concept_name}_mask"+ext)
                    image=self.mask_transforms(image)[0, None, None, ...]
                    print(placeholder_tokens.strip("<>").rstrip("0123456789"),name+f"_{concept_name}_mask"+ext,image.shape)
                    self.instance_mask_list[-1].append(image)
                    non_sense-=image
                else:
                    
                    if concept_name=="nonsense":
                        print(concept_name,"use_complement",non_sense.sum())
                        self.instance_mask_list[-1].append(non_sense)
                    else:
                        print(concept_name,"non_mask",name+f"_{concept_name}_mask"+ext)
                        self.instance_mask_list[-1].append(None)
                    

        print(self.placeholder_tokens_length_list,[j is not None for i in self.instance_mask_list for j in i],"sxc_mask_path")
        self._length = 1
    ...
    def __getitem__(self, index):
        example = {}
        chosen_index=index % len(self.instance_image_list)

        placeholder_tokens=self.placeholder_tokens_list[chosen_index]

        num_of_tokens = random.randrange(1, len(placeholder_tokens) + 1)
        tokens_ids_to_use = list(range(len(placeholder_tokens)))
        if self.shuffle:
            tokens_ids_to_use = random.sample(
                range(len(placeholder_tokens)), k=num_of_tokens
            )
        tokens_to_use = [placeholder_tokens[tkn_i] for tkn_i in tokens_ids_to_use]

        # prompt = self.customized_prompt_list[chosen_index]
        prompt = " ".join(tokens_to_use)

        for token in tokens_to_use:
            assert token in prompt       
        

        example["instance_masks"] = [self.instance_mask_list[chosen_index][i] for i in list(range(len(placeholder_tokens)))]

        example["instance_images"] = self.instance_image_list[chosen_index]
        ...
        example["chosen_index"] = chosen_index
...
def collate_fn(examples, with_prior_preservation=False):
    ...
    masks = [example["instance_masks"] for example in examples] ##maybe None in masks
    token_ids = [example["token_ids"] for example in examples]
    chosen_indexs=[example["chosen_index"] for example in examples]
...
class SpatialDreambooth:
    ...
    def main(self):
        ...
        self.args.instance_prompt_list =self.args.customized_prompt_list
        ...
        self.placeholder_tokens_list=[]
        for prompt in self.args.customized_prompt_list:
            self.placeholder_tokens_list.append(get_customized_prompt_from_prompt(prompt))
       
        self.placeholder_token_ids_list=[]
        token_index_map={}
        self.num_concepts=0
        for placeholder_tokens in self.placeholder_tokens_list:
            if len(placeholder_tokens)>0:
                self.num_concepts+=len(placeholder_tokens)
                num_added_tokens = self.tokenizer.add_tokens(placeholder_tokens)
                assert num_added_tokens==len(placeholder_tokens)
                placeholder_token_ids = self.tokenizer.convert_tokens_to_ids(
                    placeholder_tokens
                )
                for t,id in zip(placeholder_tokens,placeholder_token_ids):
                    token_index_map[t]=id
                self.text_encoder.resize_token_embeddings(len(self.tokenizer))
                
                # Initialize new tokens randomly
                token_embeds = self.text_encoder.get_input_embeddings().weight.data
                token_embeds[-len(placeholder_tokens) :] = token_embeds[
                    -3 * len(placeholder_tokens) : -2 * len(placeholder_tokens)
                ]

                self.placeholder_token_ids_list.append(placeholder_token_ids)
            else:
                self.placeholder_token_ids_list.append([])
        
        if len(self.args.initializer_tokens) > 0:
            # Use initializer tokens
            token_embeds = self.text_encoder.get_input_embeddings().weight.data
            for token_name_and_initializer_token in self.args.initializer_tokens:
                token_name,initializer_token=token_name_and_initializer_token.split(",")
                curr_token_ids = self.tokenizer.encode(
                    initializer_token, add_special_tokens=False
                )
                # assert (len(curr_token_ids)) == 1
                token_embeds[token_index_map[token_name]] = token_embeds[
                    curr_token_ids[0]
                ]
                print(token_name,initializer_token,"hh")
        
        print(self.args.customized_prompt_list,self.placeholder_tokens_list,self.placeholder_token_ids_list,self.num_concepts,"sxc")
        ...
        train_dataset = DreamBoothDataset(
            instance_img_path_list=self.args.instance_img_path_list,
            customized_prompt_list=self.args.customized_prompt_list,
            ...
            flip_p=self.args.flip_p,
            shuffle=self.args.apply_masked_loss
        )
            ...
            for step, batch in enumerate(train_dataloader):
                ...
                with self.accelerator.accumulate(self.unet):
                    ...
                    if self.args.with_prior_preservation:
                        ...
                        if self.args.apply_masked_loss:
                            instances_masks=[[batch["instance_masks"][_batch_id][i] 
                                                if batch["instance_masks"][_batch_id][i] is not None 
                                                else torch.ones([1,1,512,512]).to(model_pred) 
                                                for i in token_ids ] 
                                             for _batch_id,token_ids in enumerate(batch["token_ids"])]

                            instances_masks=[torch.cat(mask,axis=1) for mask in instances_masks]
                            
                            instances_masks=torch.cat(
                                [
                                    torch.max(masks, axis=1,keepdim=True).values
                                    for masks in instances_masks
                                ],
                                axis=0
                            )
                            
                            print(batch["token_ids"],[i.sum() for i in instances_masks])
                            max_masks =instances_masks
                            # max_masks = torch.max(
                        ...
                        if self.args.apply_masked_loss:
                            instances_masks=[[batch["instance_masks"][_batch_id][i]
                                            for i in token_ids if batch["instance_masks"][_batch_id][i] is not None] 
                                            for _batch_id,token_ids in enumerate(batch["token_ids"])]

                            instances_masks=[torch.cat(mask,axis=1) if len(mask)>0 else torch.ones([1,1,512,512]).to(model_pred) for mask in instances_masks]
                            # print(batch["token_ids"],[i.shape for i in instances_masks],[len(i) for i in batch["instance_masks"]])
                            instances_masks=torch.cat(
                                [
                                    torch.max(masks, axis=1,keepdim=True).values
                                    for masks in instances_masks
                    ...
                    #             input=mask, size=(16, 16)
                    #         ) if mask is not None else None for mask in batch["instance_masks"][batch_idx]]
                            
                    #         # print(model_pred.shape,"sxc_debug")
                    ...
                    #         print("mask_ids:",batch["token_ids"][batch_idx])

                    #         for mask_id in batch["token_ids"][batch_idx]:
                    #             if GT_masks[mask_id] is None:
                    #                 print(mask_id,"debug")
                    #                 continue
                                
                    #             curr_placeholder_token_id = self.placeholder_token_ids_list[
                    #                 batch["chosen_indexs"][batch_idx]
                    #             ][mask_id]
                    ...
                    #             )
                    #             # print(GT_masks[mask_id].shape,asset_attn_mask.shape,"sxc_debug")
                    ...
                    if self.args.lambda_attention != 0:
                        ...
                        for batch_idx in range(self.args.train_batch_size):
                            ...
                            print(batch["instance_prompts"],"mask_ids:",batch["token_ids"][batch_idx])
                            for mask_id in batch["token_ids"][batch_idx]:
                                if GT_masks[mask_id] is None:
                                    print("debug",mask_id)
                                    continue
                ...
                if self.accelerator.sync_gradients:
                    ...
                    if (
                        ...
                        #         ),
                        #     )
                        for _prompt in self.args.instance_prompt_list:
                            self.controller.cur_step = 0
                            self.controller.attention_store = {}
                            ...
                            self.perform_full_inference(
                                path=os.path.join(
                                    img_logs_path, f"{_prompt}_{global_step:05}_full_pred.jpg"
                                ),
                                instance_prompt=_prompt
                            )
                            ...
                            self.save_cross_attention_vis(
                                ...
                                path=os.path.join(
                                    img_logs_path, f"{_prompt}_{global_step:05}_full_attn.jpg"
                                ),
                            )
                            self.controller.cur_step = 0
                            self.controller.attention_store = {}
...
class P2PCrossAttnProcessor:
    ...
    def get_attention_scores(self, atten,query, key, attention_mask=None):
        dtype = query.dtype
        if atten.upcast_attention:
            query = query.float()
            key = key.float()
        ...
        # attention_scores[...,4]+=5

        if atten.upcast_softmax:
