...
from concept_utils.relation_words import  relation_words
from concept_utils.stop_words import  stop_words
...
def calculate_steer_loss(token_embedding,
    ...
    batch_loss_list=[]
    batch_loss=0
    # print("haha",batch_placeholder_token_id_list,batch_positive_ids_list)
    if with_prior_preservation:
        input_ids=input_ids[len(input_ids)//2:]
        # print("haha:",input_ids)
        # batch_positive_ids_list=batch_positive_ids_list[len(batch_positive_ids_list)//2:]
    # print("haha2:",input_ids)
    for placeholder_token_id_list,positive_ids_list in zip(batch_placeholder_token_id_list,batch_positive_ids_list):
        loss_list=[]
        # print("haha:",placeholder_token_id_list,positive_ids_list)
        # if (not positive_ids_list) or (not placeholder_token_id_list):
        #     continue
        for placeholder_token_id,positive_ids in zip(placeholder_token_id_list,positive_ids_list):
            ...
            with torch.no_grad(
                ...
                # exit(0)
                # print(placeholder_token_id_list)
                ...
                ori_positive_embeds=positive_embeds
                ...
                if positive_embeds.shape[0]==0:
                    print("pre_positive_embeds:",positive_embeds.shape[0])
                    zero_mask=torch.isin(positive_ids,
                                        torch.tensor([0]).cuda()).cuda()
                    stop_mask=zero_mask
                    positive_embeds = ori_positive_embeds[
                        ~stop_mask]
            ...
            # print("placeholder_token_id:",placeholder_token_id)
            relation_embeds = inputs_embeds[0][relation_mask]  # (1, 1024)
            ...
            loss_list.append(torch.mean(denominator - nominator))
            batch_loss+=loss_list[-1]
        batch_loss_list.append(loss_list)
    batch_loss/=sum([len(ll) for ll in batch_loss_list])
    # exit(0)
    return loss_list,batch_loss
...
def parse_args(input_args=None):
    ...
    parser.add_argument("--num_of_concepts", type=int, default=1)
    parser.add_argument("--use_postive_ids", action="store_true")
    parser.add_argument("--num_positives", type=int,default=4)
    parser.add_argument("--relation_words_path", type=str,default="")
    ...
    parser.add_argument("--sd_loss_weight",type=float,default=1)
    ...
    # assert len(args.initializer_tokens) == 0 or len(args.initializer_tokens) == args.num_of_concepts
...
class ConceptDreamBoothDataset(Dataset):
    """
    A dataset to prepare the instance and class images with the prompts for fine-tuning the model.
    It pre-processes the images and the tokenizes prompts.
    """
    ...
    def __init__(
        ...
        center_crop=False,
        flip_p=0.5,
        use_postive_ids=False,
        relation_words_list=[], ##[[]*len(placeholder_tokens)]
        num_positives=0,
        do_shuffle=False,
        suffix_list=[],
        seperator=" "
        ...
        self.use_postive_ids=use_postive_ids
        self.relation_words_list = relation_words_list
        self.num_positives = num_positives

        assert synonym is None
        self.synonym=synonym
        if self.synonym is not None and not isinstance(self.synonym,list):
        ...
        instance_mask_path = os.path.join(instance_data_root, f"mask{0}.png")
        curr_mask = Image.open(instance_mask_path)
        curr_mask = self.mask_transforms(curr_mask)[0, None, None, ...]
        self.instance_masks.append(curr_mask)
        self.instance_masks = torch.cat(self.instance_masks)
        # print(self.instance_masks.shape,"sxc0")
    ...
    def __getitem__(self, index):
        example = {}
        tokens_to_use_idx=list(range(len(self.placeholder_tokens)))
        
        if self.do_shuffle:
            random.shuffle(tokens_to_use_idx)

        tokens_to_use = [self.placeholder_tokens[i] for i in tokens_to_use_idx]
        # print("haha",tokens_to_use_idx,tokens_to_use)
        if self.synonym is not None and self.use_postive_ids:
            raise NotImplementedError("not implement")
        
        if self.synonym is not None:
            prompt = random.choice(self.synonym)
        else:
            if self.suffix_list:
                suffix_list=[self.suffix_list[i] for i in tokens_to_use_idx]
                full_token_list=list(zip(suffix_list,tokens_to_use))
                full_token_list=[" ".join([placeholder,suffix]) for suffix,placeholder in full_token_list]
                prompt="a photo of " + self.seperator.join(
                    full_token_list
                )
            else:
                prompt = "a photo of " + ", ".join(tokens_to_use)
        # print(prompt,"sxc")
        example["instance_images"] = self.instance_image
        example["instance_masks"] = self.instance_masks
        # print(example["instance_masks"].shape)
        example["token_ids"] = torch.tensor(
            [self.tokenizer.encode(token)[1] 
                for token in tokens_to_use])
        # print(tokens_to_use)
        # print("sxchaha:",example["token_ids"])
        ...
        if self.use_postive_ids:
            if self.num_positives > 0:
                assert len(self.relation_words_list)==len(self.placeholder_tokens)
                print(self.relation_words_list,self.placeholder_tokens)
                example["positive_ids"]=[]
                example["positive_prompt"] = []
                
                for _token_idx in tokens_to_use_idx:
                    relation_words=self.relation_words_list[_token_idx]
                    if isinstance(relation_words,str):
                        relation_words = [relation_words]
                    positive_words = random.sample(
                        relation_words, k=min(self.num_positives,len(relation_words)))
                    positive_words_string = " ".join(positive_words)
                    example["positive_ids"].append(self.tokenizer(
                        positive_words_string,
                        padding="max_length",
                        truncation=True,
                        max_length=self.tokenizer.model_max_length,
                    ...
                    example["positive_prompt"].append(positive_words_string)
...
class MultiConceptDreamBoothDataset(Dataset):
    ...
    def __getitem__(self, index):
        example = {}
        num_of_tokens = random.randrange(2, len(self.placeholder_tokens) + 1)

        tokens_ids_to_use = random.sample(
            range(len(self.placeholder_tokens)), k=num_of_tokens
        )
        if self.do_shuffle:
            random.shuffle(tokens_ids_to_use)

        tokens_to_use = [self.placeholder_tokens[tkn_i] for tkn_i in tokens_ids_to_use]
        
        if self.suffix_list:
            suffix_list=[self.suffix_list[i] for i in tokens_ids_to_use]
            full_token_list=list(zip(suffix_list,tokens_to_use))
            full_token_list=[" ".join([placeholder,suffix]) for suffix,placeholder in full_token_list]
            prompt="a photo of " + self.seperator.join(
                full_token_list
        ...
        if self.use_postive_ids:
            if self.num_positives > 0:
                ...
                relation_words_list=[self.relation_words_list[i] for i in tokens_ids_to_use]
                for relation_words in relation_words_list:
                    if isinstance(relation_words,str):
                        relation_words = [relation_words]
                    positive_words = random.sample(
                        relation_words, k=min(self.num_positives,len(relation_words)))
...
def collate_fn(examples, with_prior_preservation=False,use_postive_ids=False):
    ...
    instance_prompt=[example["instance_prompt"] for example in examples]
    ...
    if use_postive_ids:
        ...
        positive_ids=[[] for _ in examples]
        positive_prompt=[[] for example in examples]

    if with_prior_preservation:
        input_ids = [example["class_prompt_ids"] for example in examples] + input_ids
        
        instance_prompt= [example["class_prompt"] for example in examples] + instance_prompt
        # positive_ids=[[] for example in examples]+positive_ids
        # positive_prompt=[[] for example in examples]+positive_prompt
...
class SpatialDreambooth:
    ...
    def main(self):
        ...
        self.args.relation_words_list=[]
        if self.args.relation_words_path and os.path.exists(self.args.relation_words_path):
            import json
            with open(self.args.relation_words_path) as f:
                info=json.load(f)
                for i in range(self.args.num_of_concepts):
                    if f"<asset{i}>" in info:
                        self.args.relation_words_list.append(info[f"<asset{i}>"])
                    else:
                        self.args.relation_words_list.append([])
            print("relation_words_list:",self.args.relation_words_list)
        ...
        self.placeholder_tokens = [
            ...
            for idx in range(self.args.num_of_concepts)
        ]

        print(self.placeholder_tokens,self.args.num_of_concepts)
        num_added_tokens = self.tokenizer.add_tokens(self.placeholder_tokens)
        assert num_added_tokens == self.args.num_of_concepts
        self.placeholder_token_ids = self.tokenizer.convert_tokens_to_ids(
            self.placeholder_tokens
        )
        self.text_encoder.resize_token_embeddings(len(self.tokenizer))
        # if not self.args.customized_prompt:
        #     self.args.instance_prompt = "a photo of " + ", ".join(
        #         self.placeholder_tokens
        #     )
        # else:
        #     self.args.instance_prompt = self.args.customized_prompt

        if len(self.args.suffix_token_list)>0:
            assert len(self.args.suffix_token_list)==len(self.placeholder_tokens)
            full_token_list=list(zip(self.args.suffix_token_list,self.placeholder_tokens))
            full_token_list=[" ".join([placeholder,suffix]) for suffix,placeholder in full_token_list]
            self.args.instance_prompt="a photo of " + self.args.seperator.join(
                full_token_list
            )
        else:
            self.args.instance_prompt = "a photo of " + self.args.seperator.join(
        ...
        if len(self.args.initializer_tokens) > 0:
            ...
            token_embeds = self.text_encoder.get_input_embeddings().weight.data
            token_embeds[-self.args.num_of_concepts :] = token_embeds[
                -3 * self.args.num_of_concepts : -2 * self.args.num_of_concepts
            ]
        ...
        train_dataset = ConceptDreamBoothDataset(
            instance_data_root=self.args.instance_data_dir,
            placeholder_tokens=self.placeholder_tokens,
            # synonym=self.args.instance_prompt,
            ...
            use_postive_ids=self.args.use_postive_ids,
            relation_words_list=self.args.relation_words_list, ##[[]*len(placeholder_tokens)]
            num_positives=self.args.num_positives,
            do_shuffle=self.args.do_shuffle,
            seperator=self.args.seperator,
            suffix_list=self.args.suffix_token_list
        )
        if self.args.use_random_combine:
        ...
        train_dataloader = torch.utils.data.DataLoader(
            ...
            collate_fn=lambda examples: collate_fn(
                examples, self.args.with_prior_preservation,use_postive_ids=self.args.use_postive_ids
            ),
        ...
        for epoch in range(first_epoch, self.args.num_train_epochs):
            ...
            for step, batch in enumerate(train_dataloader):
                print("instance_prompt:",batch["instance_prompt"],"token_id:",batch["token_ids"])
                ...
                with self.accelerator.accumulate(self.unet):
                    ...
                    if self.args.with_prior_preservation:
                        ...
                        logs["sd_loss"]=sd_loss.detach().item()
                        # Compute prior loss
                        ...
                        loss = self.args.sd_loss_weight*sd_loss + self.args.prior_loss_weight * prior_loss
                    else:
                        ...
                        if self.args.apply_masked_loss:
                            ...
                            model_pred = model_pred * downsampled_mask
                            target = target * downsampled_mask
                        sd_loss = self.args.sd_loss_weight*F.mse_loss(
                            model_pred.float(), target.float(), reduction="mean"
                        )
                        loss=sd_loss
                        logs["sd_loss"]=sd_loss.detach().item()
                    ...
                    if  self.args.use_postive_ids and self.args.steer_loss_weight>0:
                        placeholder_token_ids_list=batch["token_ids"]
                        # for i in range(self.use_pre_suffix.num_of_concepts):
                        #     placeholder_token_ids = self.tokenizer.encode(
                        #         f"<asset{i}>", add_special_tokens=False
                        #     )
                        ...
                        steer_loss_list,batch_steer_loss= calculate_steer_loss(
                        ...
                        logs["steer_loss"] = weighted_steer_loss.detach().item()
                ...
                if self.accelerator.sync_gradients:
                    ...
                    if (
                        ...
                        # and global_step > self.args.phase1_train_steps ##sxc modify
                    ):
