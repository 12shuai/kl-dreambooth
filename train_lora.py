...
from my_lora_processor import MyLoRACrossAttnProcessor
...
class SpatialDreambooth:
    ...
    def main(self):
        ...
        for name in self.unet.attn_processors.keys():
            ...
            lora_attn_procs[name] = MyLoRACrossAttnProcessor(
                hidden_size=hidden_size, cross_attention_dim=cross_attention_dim,rank=128
            )
            

        self.unet.set_attn_processor(lora_attn_procs)
        self.lora_layers = AttnProcsLayers(self.unet.attn_processors)#.to(self.accelerator.device)
        # print("sxc:",next(self.lora_layers.parameters()).device)
        self.lora_layers.requires_grad_(False)
        total_params = sum(p.numel() for p in self.lora_layers.parameters())

        total_params2=sum([i.numel() for i in self.text_encoder.parameters()])
        print(f"Number of Trainable Parameters: {total_params * 1.e-6:.2f} M")
        print(f"Number of Trainable Parameters: {total_params2 * 1.e-6:.2f} M")
        # exit(0)
        ...
        (
            ...
            self.lora_layers,self.unet,self.text_encoder, optimizer, train_dataloader, lr_scheduler
        )
        ...
        for epoch in range(first_epoch, self.args.num_train_epochs):
            ...
            for step, batch in enumerate(train_dataloader):
                if self.args.phase1_train_steps == global_step:
                    ...
                    # assert not self.args.train_text_encoder, "not implemented"
                    if self.args.train_text_encoder:
                ...
                with self.accelerator.accumulate(self.unet):
                    ...
                    if self.accelerator.sync_gradients:
                        ...
                        # )
                        # assert not self.args.train_text_encoder, "not implement!!!"
                        params_to_clip =(
                            itertools.chain(
    ...
    def save_pipeline(self, path):
        ...
        if self.accelerator.is_main_process:
            ...
            self.unet.save_attn_procs(path)
            pipeline = DiffusionPipeline.from_pretrained(
                self.args.pretrained_model_name_or_path,
